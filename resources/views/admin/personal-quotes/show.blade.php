...
<div class="p-6 max-w-7xl mx-auto mt-24 text-white space-y-6">
    <h1 class="text-3xl font-bold mb-6">Personal Request #{{ $quote->id }}</h1>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="bg-zinc-800 p-6 rounded-lg shadow flex-1">
            ...
            <ul class="space-y-1">
                <li><strong>Name:</strong> {{ $quote->name }} {{ $quote->lastname }}</li>
                <li><strong>Phone:</strong> {{ $quote->phone }}</li>
                <li><strong>Email:</strong> {{ $quote->email }}</li>
                <li><strong>Address:</strong> {{ $quote->address }}</li>
                <li><strong>Date of Birth:</strong> {{ $quote->dob }}</li>
                <li><strong>Occupation:</strong> {{ $quote->occupation }}</li>
            </ul>
        </div>
        ...
        <div class="bg-zinc-800 p-6 rounded-lg shadow flex-1">
            ...
            <ul class="space-y-1">
                <li><strong>VIN(s):</strong> {{ $quote->vin }}</li>
                <li><strong>Miles:</strong> {{ $quote->miles ?? 'N/A' }}</li>
                <li><strong>Coverage:</strong> {{ $quote->coverage }}</li>
                <li><strong>Vehicle:</strong> {{ $quote->make }} {{ $quote->model }} ({{ $quote->vehicle_type }} - {{ $quote->body_class }})</li>
                <li><strong>Usage:</strong> {{ $quote->usage ?? 'N/A' }}</li>
                <li><strong>Observations:</strong> {{ $quote->observations ?? 'N/A' }}</li>
            </ul>
        </div>
    </div>
    ...
    <!-- Botón back -->
    <div class="mt-6">
        <a href="{{ route('admin.personal-quotes.index') }}" class="text-gray-400 hover:underline">← Back to list</a>
    </div>
</div>
